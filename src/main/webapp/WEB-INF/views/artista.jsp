...
<div align="center" style="margin-top: 3%;">
    ...
    <c:choose>
        <c:when test="${artistaPesquisado!=null}">
            <h1 style="color: grey; font-weight: bold;
                font-size: 460%;">${artistaPesquisado.getNome()}</h1>
        </c:when>
        <c:otherwise>
            <h1 style="color: grey; font-weight: bold;
                font-size: 460%;">artistaPesquisado == null</h1>
        </c:otherwise>
    </c:choose>
</div>
...
<%--       // <c:forEach var="artista" items="${artistas}">--%>
            <tr style="margin-top: 10%;">
                <th scope="row" style="font-weight: bolder;">${artistaPesquisado.getNome()}</th>
                <c:choose>
                    <c:when test="${!isGerente}">
                        <td align="center">
                            <div>
                        ...
                        <td align="center">
                            <a style="font-weight: bolder; color: hotpink; font-style: normal"
                               href="/obras/obras-artista.action?id=${artistaPesquisado.getId()}">
                                VER OBRAS
                            </a>
                        </td>
                    </c:when>
